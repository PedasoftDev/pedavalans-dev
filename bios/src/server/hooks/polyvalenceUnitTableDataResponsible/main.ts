...
namespace PolyvalenceUnitTableDataResponsible {
    ...
    export const GetByPolyvalenceUnitId = (polyvalence_table_id: string): { dataResponsible: IPolyvalenceUnitTableDataResponsible.IBase[], isLoadingDataResponsible: boolean } => {
        const { documents, isLoading } = useListDocuments(AppInfo.Name, AppInfo.Database, Collections.PolyvalenceUnitTableDataResponsible, [Query.limit(10000), Query.equal("polyvalence_table_id", polyvalence_table_id), Query.equal("is_deleted", false)])
        return {
    ...
    export const GetListByAccountId = (account_id: string): { dataResponsible: IPolyvalenceUnitTableDataResponsible.IBase[], isLoadingDataResponsible: boolean } => {
        const { documents, isLoading } = useListDocuments(AppInfo.Name, AppInfo.Database, Collections.PolyvalenceUnitTableDataResponsible, [Query.limit(10000), Query.equal("responsible_employee_id", account_id), Query.equal("is_deleted", false)])
        return {
            dataResponsible: documents as any,
            isLoadingDataResponsible: isLoading
        }
