...
export function getReportToExcelByMachinePolyvalenceTable(machines: IMachine.IBase[], competencyMachineAssocation: ICompetencyMachineAssociation.IBase[], competenciesDb: ICompetency.ICompetency[], competencyGroups: ICompetencyGroup.IGetCompetencyGroup[], employeeCompetencyValues: IEmployeeCompetencyValue.IEmployeeCompetencyValue[], polyvalence_table_name: string) {
    ...
    competencyGroupRow[0] = { v: '', t: 's', s: { ...headerStyle, border: borderStyles.header.topBottomLeft } }
    ...
    //// durum -- yazma
    competenciesRow[2] = { v: 'Durum', t: 's', s: { ...headerStyle, border: borderStyles.header.topBottomLeft } }
    competenciesRow[3] = { v: '', t: 's', s: { ...headerStyle, border: borderStyles.header.topBottomRight } }
    //// durum -- merge
    merge.push({ s: { r: 3, c: 2 }, e: { r: 3, c: 3 } })
    ...
    competenciesRow[startCompetencyIndex + competencies.length] = { v: 'Beklentiyi Karşılama Oranı', t: 's', s: { ...headerStyle, border: borderStyles.header.topBottomRight, alignment: { wrapText: true, ...alignCenter } } }
    colWidths[startCompetencyIndex + competencies.length] = { wpx: 150 };

    competenciesRow[startCompetencyIndex + competencies.length + 1] = { v: 'Geliştirilmesi Gereken Yetkinlik Sayısı', t: 's', s: { ...headerStyle, border: borderStyles.header.topBottomRight, alignment: { wrapText: true, ...alignCenter } } }
    ...
    competencyGroups.forEach((group, index) => {
        ...
        appendData[employees.length * 2 + 8 + index * 2] = competencyGroupPercentageRow;
    })
