...
export class TitleListController extends UIController {
  ...
  public LoadView(): UIView {
    ...
    return (
      isLoading || isLoadingResult || isLoadingTitles ? VStack(Spinner()) :
        UIViewBuilder(() => {
            ...
            },
            {
              ...
              renderCell: (params: any) => (
                <Button variant='text' onClick={() => {
                  navigate(Resources.OrganizationStructureTabValues.find(x => x.value === 1)?.link + "/" + params.row.$id);
                }}>Düzenle</Button>
              )
            }
          ];
          ...
          return (
            VStack({ alignment: cTop })(
              HStack({ alignment: cLeading })(
                Views.Title(active ? "Organizasyon Yapısı" : "Organizasyon Yapısı / Pasif Ünvanlar").paddingTop("20px")
                    ...
                    <TabPanel value={value} index={1}>
                      <div style={{ display: "flex", flexDirection: "column", padding: "8px 0", gap: "5px" }
                        ...
                        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                          <div style={{ width: "60%" }}>
                            <TextField size='small' label='Ünvan Arayın' variant='outlined' fullWidth onChange={(e) => setFilterKey(e.target.value)} />
                          </div>
                          <div style={{ width: "10%" }}>
                            {
                              <Tooltip title={`${active ? "Pasif" : "Aktif"} Ünvanları Göster`}>
                        ...
                        <GridContainer>
                          <StyledDataGrid
                            rows={titles.filter(x => x.is_active === active).filter(x => x.name.toLowerCase().indexOf(filterKey.toLowerCase()) > -1)}
                            columns={columns} />
                        </GridContainer>
                      </div>
                    </TabPanel>
