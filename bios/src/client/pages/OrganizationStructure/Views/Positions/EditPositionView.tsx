...
import { ReactView, Spinner, UIViewBuilder, VStack, nanoid, useParams } from '@tuval/forms';
...
const EditPositionView = (
    ...
    const { positionVocationalQualificationRelationList, isLoading: isLoadingPositionVQRelationList } = PositionVocationalQualificationRelation.ListByPosition(form.id)

    return (

        <Fragment>
            {VStack(
                isLoading || isLoadingOrganizationStructurePosition || isLoadingDocument || isLoadingPositionVQRelationList || isLoadingPositionsList ?
                    VStack(Spinner()) :
                    UIViewBuilder(() => {
                        const [selectedDocuments, setSelectedDocuments] = useState([])
                        useEffect(() => {
                            setSelectedDocuments(positionVocationalQualificationRelationList.map((x) => x.document_id))
                        }, [])

                        const onSubmit = (e: React.FormEvent<HTMLFormElement>) => {
                            e.preventDefault();
                            const havePosition = positionsList.find((position) => position.$id != form.id && position.record_id === form.record_id)
                            if (havePosition) {
                            ...
                            update({
                                ...
                                if (selectedDocuments.length != 0) {
                                    ...
                                    selectedDocuments.forEach((document_id, i) => {
                                        const nanoId = nanoid();
                                                ...
                                                <div style={{
                                                    ...
                                                    <StyledDataGrid
                                                        ...
                                                        onRowSelectionModelChange={(newRowSelectionModel: any) => {
                                                            setSelectedDocuments(newRowSelectionModel)
                                                        }}
