...
class PedavalansService extends RealmoceanService {
  ...
  async checkTargetDateForReminder() {
    ...
    // const competencies: ICompetency[] = await this.databaseService.listDocuments(this.appName, this.databaseName, this.Competency, [queryLimit]).then(x => x.documents);
    const competenciesWithDepartment: ICompetencyDepartment[] = await this.databaseService.listDocuments(this.appName, this.databaseName, this.CompetencyDepartment, [queryLimit]).then(x => x.documents);
    if (reminderDayCount > 0) {
      ...
      for (let i = 0; i < polyvalenceTables.length; i++) {
        ...
        const competenciesWithDepartmentByTable = competenciesWithDepartment.filter(x => x.competency_department_table_id === table.$id);
        ...
        if (reminderDate.getTime() === today.getTime()) {
          ...
          if (authParameterIsActive) {
            responsibleUsers.forEach(async (responsibleUser) => {
              ...
              if (account) {
                try {
                  const htmlEmployeeList = mailTemplate(employeesByTable.map(x => x?.first_name + ' ' + x?.last_name));
                  await this.emailSender(account.mail, "Polivalans Tablosu Veri Girişi Hatırlatma", htmlEmployeeList, {
            ...
            accountRelations.forEach(async (account) => {
              try {
                // await this.emailSender(account.mail, "Polivalans Tablosu Veri Girişi Hatırlatma", "Polivalans tablosu hedef veri girişi yapmanız gerekmektedir, {{gun}} kaldı. {{calisanlar}}", {
                //   calisanlar: tableEmployees.map(x => x.first_name + ' ' + x.last_name).join(', '),
                //   gun: reminderDayCount
                // })
              } catch (error) {
                console.log(error);
              }
  ...
  async getNextPeriodFromCurrentDate(frequency: string) {
    ...
    if (frequency === 'Yıl') {
      ...
      return { period: `${currentMonth + 1} Ayı Dönemi`, startDate, endDate };
    }
    return null;
  }
